...
#define TEST_LED_DUTY_CYCLE 	4
#define TEST_WATCHDOG_TIMEOUT	5
#define TEST_UART_WITH_INTR		6
...
#define __IWDG_PERIOD             0x001E8480	//2000000 - 2 secs
#define __IWDG_PR             (2)
...
void Display_Test_Menu(void) {
	printf("\n\r0: Test vector interrupt\n\r");
	printf("1: Test ADC tone\n\r");
	printf("2: Test poll interrupt\n\r");
	printf("3: Test LED output\n\r");
	printf("4: Test LED duty cycle\n\r");
	printf("5: Test watchdog timeout\n\r");
	printf("6: Test UART with interrupt\n\n\r");
	printf("To select a test enter its test number\n\r");
	printf("To exit the selected test press the Reset button\n\r");
}
...
int main (void) {
	...
	//unsigned char testSel = TEST_LED_DUTY_CYCLE;
	unsigned char testSel = TEST_WATCHDOG_TIMEOUT;
	//unsigned char testSel = TEST_UART_WITH_INTR;
	...
	if (!(RCC->CSR & (1<<29))) {
		...
		while (TRUE) {
			...
			testSel = chr - 48;
	   		printf("\n\r");
			if (testSel >= 0 && testSel <= 6) break;
			printf("Test number not valid - try again...\n\r");
		}
	}
	...
	switch (testSel) {
		...
		case TEST_WATCHDOG_TIMEOUT:
  			if (RCC->CSR & (1<<29)) {                       	// IWDG Reset Flag set, i.e. reset is due to watchdog
    			RCC->CSR |= (1<<24);                          	// Clear Reset Flags
				displayTestMessage(5, 0, "Wdog reset!", 0);
  			}	 
			stm32_IwdgSetup();

			displayTestMessage(3, 0, "Wdog Timeout", testSel);
			while (TRUE) {
				//IWDG->KR  = 0xAAAA;                    // reload the watchdog
			}
			break;
			...
			while (TRUE) {  //Loop forever
				...
				switch (joystick_int) {
					...
					case JOYSTICK_SELECT:
						...
						generate_FSK_Tone("Hello there friends, how are you today!");
		...
		case TEST_LED_DUTY_CYCLE:
			displayTestMessage(3, 0, "LED Duty Cycle:", testSel);
			AD_done = 0;
			ADC1->CR2 |= (1UL << 22);       		//Start the ADC conversion
			doTone = 0;
			ledMk1 = 100;
			...
			while (TRUE) {
				...
				if (AD_done) {
					...
					if (doTone) {
						...
						//mark/space ration which decides the duty
						//cycle for the electrical energy to the LEDs
						ledMk1 = c;
						ledMk2 = (100 - c);
					}
					AD_done = 0;  					//Reset the ADC complete flag waiting for next sample
				...
				if (doTone) {
					//Pulse width modulation is used to dim or brighten the lights
					//The mean enery level is decided by the amount of time the LEDs
					//are kept switched on.
					All_LEDs_On();   	//LEDs on
					delay10th(ledMk1);
